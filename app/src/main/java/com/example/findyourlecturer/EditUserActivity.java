...
/*
    public void bteditt(View view) {
        ...
        if(pilihuser.equals("Dosen")) {
            ...
            refdosen.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    Toast.makeText(EditUserActivity.this, "Update Data Berhasil", Toast.LENGTH_SHORT).show();
...
*/
/*
            FirebaseUser userdosen = FirebaseAuth.getInstance().getCurrentUser();
            AuthCredential credential = EmailAuthProvider.getCredential("email", "password");

            userdosen.reauthenticate(credential).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    userdosen.updateEmail(editemail.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(EditUserActivity.this, "Update Email Berhasil", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(EditUserActivity.this, "Update Email Gagal", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
*//*
...
*/
/*
                    userdosen.updatePassword(editpsswrd.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(EditUserActivity.this, "Update Password Berhasil", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(EditUserActivity.this, "Update Password Gagal", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
*//*
...
*//*
            ...
            DatabaseReference refmhs = FirebaseDatabase.getInstance().getReference("user").child("Mahasiswa").child(kunci);
            refmhs.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    dataSnapshot.getRef().child("noidentitas").setValue(editno.getText().toString());
                    dataSnapshot.getRef().child("namalengkap").setValue(editnama.getText().toString());
